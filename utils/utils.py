...
def sanity_check(args: Union[argparse.Namespace, DictConfig]):
    ...
    if "device_ids" in args:
        print("WARNING:")
        print(
            "Running feature extraction on multiple devices in a _single_ process is no longer supported."
        )
        print(
            "To use several GPUs, you simply need to start the extraction with another GPU ordinal."
        )
        print(
            'For instance, in one terminal: `device="cuda:0"` and `device="cuda:1"` in the second, etc.'
        )
        print(
            f'Your device specification (device_ids={args.device_ids}) is converted to `device="cuda:0"`.'
        )
        args.device = "cuda:0"
    if "cuda" in args.device and not torch.cuda.is_available():
        print(
            f"A GPU was attempted to use but the system does not have one. Going to use CPU..."
        )
        args.device = "cpu"
    assert (
    ...
    if args.feature_type in [
        ...
        if args.keep_tmp_files:
            print(
                "If you want to keep frames while extracting features, please create an issue"
            )
    if args.feature_type == "pwc" or (
        args.feature_type == "i3d" and args.flow_type == "pwc"
    ):
    ...
    if "extraction_fps" in args and "extraction_total" in args:
        assert not (
            args.extraction_fps is not None and args.extraction_total is not None
...
def form_list_from_user_input(
    ...
    file_with_video_paths: Union[str, None] = None,
    output_path: str = "/root/models/video_features/output_video-features_thumos_i3d/",
...
def which_ffmpeg() -> str:
    ...
    if platform.system().lower() == "windows":
        result = subprocess.run(
            ["where", "ffmpeg"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
...
def extract_wav_from_mp4(video_path: str, tmp_path: str) -> str:
    ...
    assert video_path.endswith(
        ".mp4"
    ), "The file does not end with .mp4. Comment this if expected"
